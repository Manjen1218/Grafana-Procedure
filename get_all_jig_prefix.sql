DELIMITER //
CREATE OR REPLACE PROCEDURE get_all_jig_prefix(
    IN in_db VARCHAR(64),
...
BEGIN
    ...
    read_loop: LOOP
        ...
        SET sql_query = CONCAT(
            'SELECT SUBSTRING_INDEX(jig, ''-'', 1) AS jig_prefix FROM `', in_db, '`.`', tablename, '` WHERE jig IS NOT NULL ',
    ...
    IF sql_query != '' THEN
        ...
        'SELECT DISTINCT jig_prefix FROM (', sql_query, ') AS combined ORDER BY jig_prefix'
        );
